...
router.post('/notes/new-note', isAuthenticated, async (req, res) => {
  ...
  if (errors.length > 0) {
    ...
    const newNote = new Note({title, description, price});
    newNote.user = req.user.id;
    await newNote.save();
    req.flash('success_msg', 'Se agregó su producto correctamente');
    res.redirect('/notes');
  }
});
...
router.put('/notes/edit-note/:id', isAuthenticated, async (req, res) => {
  const { title, description, price } = req.body;
  await Note.findByIdAndUpdate(req.params.id, {title, description, price}) .lean() ;
